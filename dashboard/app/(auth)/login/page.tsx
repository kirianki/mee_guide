...
function LoginForm() {
    ...
    return (
        <div className="max-w-md w-full glass-panel p-8 rounded-2xl relative overflow-hidden transition-all duration-500 hover:border-indigo-500/30 hover:shadow-indigo-500/10 hover:shadow-2xl">
            ...
            <div className="absolute top-0 right-0 -mr-20 -mt-20 w-40 h-40 bg-indigo-500/20 rounded-full blur-3xl pointer-events-none"></div>
            <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-40 h-40 bg-violet-500/20 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <div className="text-center mb-8 relative z-10">
                ...
                <p className="text-slate-400 mt-2 text-sm">Sign in to your Publisher Dashboard</p>
            </div>

            <div className="relative z-10">
                {wasRegistered && (
                    <div className="mb-6 p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm rounded-xl flex items-center gap-3">
                        <span className="text-emerald-500">✓</span> Account created successfully! Please log in.
                    </div>
                )}
...
export default function LoginPage() {
    return (
        <div className="min-h-screen bg-[#0f172a] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(120,119,198,0.3),rgba(255,255,255,0))] flex flex-col items-center justify-center p-4">
            <Suspense fallback={<div className="animate-pulse text-indigo-400">Loading auth...</div>}>
                <LoginForm />
            </Suspense>
        </div>
    );
